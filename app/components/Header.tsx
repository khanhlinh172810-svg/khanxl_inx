...
export default function Header() {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value.trim()) {
      router.push(`/timkiem?q=${encodeURIComponent(e.target.value.trim())}`);
    }
  };
      ...
      <div className="bg-red-700">
        <div className="max-w-screen-xl mx-auto px-6 py-1.5 flex items-center justify-between">
          <span className="text-xs text-red-200">
            Chào mừng đến với{" "}
            <span className="text-white font-semibold">SMARTHUB</span> — Thế
            giới công nghệ
          </span>
          <a
            href="tel:18009999"
          ...
          <div className="flex-1 relative">
            ...
            <button
              onClick={() => {
                if (keyword.trim())
                  router.push(
                    `/timkiem?q=${encodeURIComponent(keyword.trim())}`,
                  );
              }}
