...
const SECRET = process.env.JWT_SECRET || "smarthub_secret_2026";
...
exports.getMe = async (req, res) => {
  try {
    // Lấy ID từ đối tượng req.user được gán qua middleware hoàn toàn chính xác
    const userId = req.user?.id || req.userId;

    if (!userId) {
      return res
        .status(401)
        .json({ message: "Không tìm thấy thông tin định danh" });
    }
